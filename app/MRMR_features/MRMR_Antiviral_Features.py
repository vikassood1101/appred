...
#########dpc ############
def av_dpc_comp(df,q):
    result_DPC = []
    std = ["G","L","W"]
    std1 = ["K"]
    zz = df.iloc[:,0]
    desired_DPC = ["GK","LK","WK"]
    header_list = []
...
def av_qos(df,gap,w=0.1):
    ...
    return zz5.loc[:,["QSO3_G_K","QSO3_SC_D"]]

################## SER ######################

def av_SE_residue_level(df):
    data = df[0].to_list()
    GH=[]
...
################ Transition #################
    ...
    return df_Comp_Transition[["CeTD_23_CH","CeTD_23_PO","CeTD_CH3","CeTD_SA1"]] 
    


############## PAAAC ###########
def av_val(AA_1, AA_2, aa, mat):
...
def av_paac_1(df,lambdaval,w=0.05):
    ...
    for k in range(0,len(df)):
        ...
        pseudo1 = [] 
        for n in range(1,lambdaval+1):
            cc.append(sum([av_val(df[0][k][p], df[0][k][p + n], aa, dd) for p in range(len(df[0][k]) - n)]) / (len(df[0][k]) - n))
...
def av_lookup(peptide,featureNum):
    l=len(peptide);
    peptide = list(peptide);
    out=np.zeros(l);
    peptide_num = av_encode(peptide);
...
def av_phyChem(seq,mode='all',m=0,n=0):
    output = av_pcp(seq);
    return output
...
def av_sep_wp(df):
    ...
    desired_feature = ["Negatively charged","Neutral","Positively charged","Polarity"]
