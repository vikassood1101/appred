...
#####################Composition######################
    ...
    for i in header1:
        for j in range(1,4):
            head.append(i+str(j))
    df11.columns = head
    features = ["CeTD_HB1","CeTD_VW2",'CeTD_PZ1','CeTD_CH1','CeTD_CH2',"CeTD_CH3",'CeTD_SS2','CeTD_SS3','CeTD_SA1','CeTD_SA2','CeTD_SA3']
    df11 = df11.loc[:,features]
    return df11
  
################## fifth feature PAAC3###################

def NON_AMP_paac_comp(df):
    std = list("K")
    PAAC_Result = []
    zz = df.iloc[:,0]
    for j in zz:
...
#######################sixth feature APAAC3#############
def NON_AMP_apaac_comp(df):
    std = list("K")
    APAAC_Result = []
    zz = df.iloc[:,0]
    for j in zz:
